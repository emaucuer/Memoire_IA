...
list_var <- setdiff(names(dataFinaleCom_new), 
                    c("id","annee","longM","latM","inondation", "secheresse",
                      "commune", "indic_in", "indic_sec", "code_dep"))

list_var_continuous <- setdiff(list_var, c(names(dataFinaleCom_new)[startsWith(names(dataFinaleCom_new),"ppri")],
...
for (catnat in list_CN){
  ...
  val_catnat <- dataFinaleCom_new %>% select(catnat) 
  
  correlation_value <- as.data.frame(cor(val_catnat, 
                                         dataFinaleCom_new %>% select(list_var_continuous),
...
for (catnat in list_CN){
  print(catnat)
  val_catnat <- dataFinaleCom_new_pl %>% select(catnat) 
  
  correlation_value <- as.data.frame(cor(val_catnat, 
                                         dataFinaleCom_new_pl %>% select(list_var_continuous),
                                         use="complete.obs", method="spearman"))
...
dataCorVar = cor(dataFinaleCom_new %>% 
...
dataPlot_indicateur <- dataFinaleCom_new %>%
...
dataFinaleComA2_pl <- read_rds("../Data/modelisation/data_modelisation_1982_2021_all_V2_A2_pl.rds")

data_modelisation_ino <- dataFinaleComA2 %>% filter(annee > 1984)
...
data_modelisation_sec <- dataFinaleComA2_pl %>% filter(annee > 1990 & annee < 2021)
